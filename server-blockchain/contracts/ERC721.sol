...
contract ERC721 {
    ...
    function approve(address to, uint256 tokenId) public payable {
        ...
        require(
            ...
            "msg.sender is not owner approval operator"
        );
    ...
    function getApproved(uint256 tokenId) public view returns (address) {
        require(_owners[tokenId] != address(0), "Token id does not exits"); // check for standard ERC721
    ...
    function transferFrom(
        ...
        require(
            ...
            isApprovedForAll(owner, msg.sender), "Msg.sender is not the owner..."
        );

        require(owner == from , "from address is not the owner");

        require(to != address(0), "Address is the zero");
    ...
    // check implement when deploy in marketplace - EIP165 proposal
